...
use goliatone\flatg\backend\drivers\IStorageManager;

/**
 * 
 */
class DropboxDriver  implements IStorageManager
{
    public $service;
    ...
    public function bootstrap($config)
    {
        ...
        $encrypter = new Encrypter('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX');
        
        $storage = new Session($encrypter);
        
        $OAuth = new Curl($key, $secret, $storage, $callback);
        $this->service = new API($OAuth);
    ...
    public function listFiles($refresh = FALSE, $options = FALSE)
    {
        ...
        if(!$options || !isset($options['path']))
            $path = $this->folder;
        else $path = $options['options'];
        ...
        echo "<pre>LIST FILES EXCEPTION HANDLER:<BR/>";
        print_r($meta);
        echo "</pre>";
    ...
    public function sync($local = TRUE)
    {
        ...
        */
        ...
        } else echo "No PULL from store<br/>";
        ...
        } else echo "No PUSH from store<br/>";
        ...
        } else echo "No SYNC from store<br/>";
    }
}